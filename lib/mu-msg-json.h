...
static void
append_attr_list (JsonBuilder *bob, const char* elm, const GSList *lst)
{
	const GSList *cur;
...
static void
append_body_attr (JsonBuilder *bob, const char* elm, const char *str)
{
	gchar *esc;
...
static void
add_prefix_maybe (JsonBuilder *bob, gboolean *field, const char *prefix)
{
...
static void
append_contacts (JsonBuilder *bob, MuMsg *msg)
{
	ContactData cdata;
...
static void
append_flags (JsonBuilder *bob, MuMsg *msg)
{
	FlagData fdata;
...
static void
append_parts (JsonBuilder *bob, MuMsg *msg, MuMsgOptions opts)
{
	PartInfo pinfo;
...
static void
append_thread_info (JsonBuilder *bob, const MuMsgIterThreadInfo *ti)
{
...
static void
append_param (JsonBuilder *bob, const GSList *param)
{
...
static void
append_message_file_parts (JsonBuilder *bob, MuMsg *msg, MuMsgOptions opts)
{
	const char	*str;
	GError		*err;
	const GSList    *params;
...
static void
append_tags (JsonBuilder *bob, MuMsg *msg)
{
	const GSList *tags, *t;
	gchar *tagesc;
		...
		MuMsgOptions opts)
{
	...
	if (ti)
		append_thread_info (gstr, ti);

	json_builder_add_string_value (bob, "subject", mu_msg_get_subject (msg));
	...
	append_date_and_size (gstr, msg);

	json_builder_add_string_value (bob, "message-id", mu_msg_get_msgid (msg));
	json_builder_add_string_value (bob, "mailing-list",
			  mu_msg_get_mailing_list (msg));
	json_builder_add_string_value (bob, "path",	 mu_msg_get_path (msg));
	json_builder_add_string_value (bob, "maildir", mu_msg_get_maildir (msg));
	json_builder_add_string_value (bob, "priority",
				      mu_msg_prio_name(mu_msg_get_prio(msg)));
	append_flags (gstr, msg);
	append_tags  (gstr, msg);

	append_list (bob, "references", mu_msg_get_references (msg));
	json_builder_add_string_value (bob, "in-reply-to",
