...
#include "gtk/widgets/msgview/mu-msg-view.h"
...
main (int argc, char *argv[])
{
	...
	if (!mu_app_init (&argc, &argv, "mu-msgview-gtk"))
		return 1;

	gtk_init (&argc, &argv);
	if (argc < 2) {
		g_printerr ("usage: mu-msgview-gtk [OPTIONS] mailfile\n");
		mu_app_uninit ();
		return 1;
	}
	view = mu_msg_view_new ();
	...
	if (!msg) {
		g_printerr ("failed to open msg\n");
		mu_msg_gmime_uninit ();
		mu_app_uninit ();
		return 1;
	}

	mu_msg_view_set_message (MU_MSG_VIEW(view), msg);
	if (mu_msg_gmime_get_subject(msg))
